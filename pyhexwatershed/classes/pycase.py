...
from pyhexwatershed.algorithms.auxiliary.export_json_to_geojson_polyline import export_json_to_geojson_polyline, export_json_to_geojson_polyline_parallel
from pyhexwatershed.algorithms.auxiliary.export_json_to_geojson_polygon import export_json_to_geojson_polygon, export_json_to_geojson_polygon_parallel
...
class hexwatershedcase(object):
    ...
    iFlag_export_individual_watershed = 0
    ...
    if iFlag_visual is not None:
        from ._visual import plot, plot_flow_direction #, _plot_mesh_with_flow_direction
    ...
    def __init__(self, aConfig_in, iFlag_create_directory_in = 1):
        ...
        if 'iFlag_resample_method' in aConfig_in:
            self.iFlag_resample_method = int(
                aConfig_in['iFlag_resample_method'])
        ...
        if 'iFlag_export_individual_watershed' in aConfig_in:
            self.iFlag_export_individual_watershed = int(
                aConfig_in['iFlag_export_individual_watershed'])
        else:
            self.iFlag_export_individual_watershed = 0
        ...
        if 'iFlag_stream_burning_topology' in aConfig_in:
            self.iFlag_stream_burning_topology = int(
                aConfig_in['iFlag_stream_burning_topology'])
        ...
        if 'iFlag_elevation_profile' in aConfig_in:
            self.iFlag_elevation_profile = int(
                aConfig_in['iFlag_elevation_profile'])
        ...
        if 'dAccumulation_threshold' in aConfig_in:
            self.dAccumulation_threshold = float(
                aConfig_in['dAccumulation_threshold'])
        ...
        if self.iFlag_user_provided_binary == 1:
            ...
            if 'sFilename_hexwatershed_bin' in aConfig_in:
                ...
                print(
                    'The binray file is not provided. The model will use the default binary file')
                self.iFlag_user_provided_binary = 0
                pass
        else:
        ...
        self.sFilename_mesh_info = os.path.join(
            str(Path(self.sWorkspace_output_pyflowline)), sMesh_type + "_mesh_info.json")
        self.sFilename_hexwatershed_json = os.path.join(
            str(Path(self.sWorkspace_output_hexwatershed)),  "hexwatershed.json")

        # individual output variables
        self.sFilename_elevation = os.path.join(
            str(Path(self.sWorkspace_output_hexwatershed)), sMesh_type + "_elevation.geojson")
        self.sFilename_slope = os.path.join(
            str(Path(self.sWorkspace_output_hexwatershed)), sMesh_type + "_slope.geojson")
        self.sFilename_drainage_area = os.path.join(str(Path(
            self.sWorkspace_output_hexwatershed)), sMesh_type + "_drainage_area.geojson")
        self.sFilename_flow_direction = os.path.join(str(Path(
            self.sWorkspace_output_hexwatershed)), sMesh_type + "_flow_direction.geojson")
        self.sFilename_distance_to_outlet = os.path.join(str(Path(
            self.sWorkspace_output_hexwatershed)), sMesh_type + "_distance_to_outlet.geojson")

        self.sFilename_variable_polyline = os.path.join(str(Path(
            self.sWorkspace_output_hexwatershed)), sMesh_type + "_variable_polyline.geojson")
        self.sFilename_variable_polygon = os.path.join(str(Path(
            self.sWorkspace_output_hexwatershed)), sMesh_type + "_variable_polygon.geojson")

        self.sFilename_animation_json = os.path.join(
            str(Path(self.sWorkspace_output_hexwatershed)),  "animation.json")
        return
    ...
    def pyhexwatershed_setup(self):
        ...
        if self.iFlag_user_provided_binary == 1:
            ...
            sFilename_new = os.path.join(
                str(Path(self.sWorkspace_output_hexwatershed)),  sFilename_executable)
            ...
            for folder in os.environ['PATH'].split(os.pathsep):
                sFilename_hexwatershed_bin = os.path.join(
            ...
            if iFlag_found_binary == 1:
                ...
                sFilename_hexwatershed_bin = os.path.join(
                    str(Path(sPath_installation + '/pyhexwatershed/_bin/')),  sFilename_executable)
                if os.path.isfile(sFilename_hexwatershed_bin):
                    iFlag_found_binary = 1
    ...
    def pyhexwatershed_assign_elevation_to_cells(self, dMissing_value_in=None, sFilename_preprocessed=None, sField_name_in='elevation'):
        ...
        ptimer = pytimer()
        ptimer.start()
        if sFilename_preprocessed is not None:
            #check whether the file exists
            if os.path.exists(sFilename_preprocessed):
                print(f"File {sFilename_preprocessed} exists")
                iFlag_use_preprocessed = 1
                if sField_name_in is not None:
                    sField_name = sField_name_in
                else:
                    sField_name = 'elevation'
            else:
                print(f"File {sFilename_preprocessed} does not exist")
                iFlag_use_preprocessed = 0
        else:
            iFlag_use_preprocessed = 0

        if iFlag_use_preprocessed == 1:
            aCell_dict = dict()
            aCell_in = self.pPyFlowline.aCell
            #read the sFilename_preprocessed using the correct ogr driver
            #pDriver_vector = get_vector_driver_from_filename(sFilename_preprocessed)
            ...
            for idx, pCell in enumerate(aCell_in):
                if pCell.lCellID in aElevation:
                    pCell.dElevation_mean = aElevation[pCell.lCellID]
                    aCell_dict[pCell.lCellID] = idx
                else:
                    pCell.dElevation_mean = -9999.0
                    pass
            pass
        else:
            iFlag_use_cutlinewkt = 1
            #check whether cutlinewkt option is available
            if iFlag_use_cutlinewkt ==1:
                ...
                if iFlag_resample_method == 1:
                    for pCell in aCell_in:
                        ...
                        lRow_index = int((dY_top - dY_out) / abs(pPixelHeight))
                    ...
                    if iFlag_resample_method == 2:  # zonal mean
                        ...
                        for pCell in aCell_in:
                            ...
                            for j in range(nVertex):
                                x1 = pCell.aVertex[j].dLongitude_degree
                                y1 = pCell.aVertex[j].dLatitude_degree
                                ring.AddPoint(x1, y1)
                                pass
                            ring.closeRings()
                            ...
                            if (pProjection_target != pProjection_wgs84):
                                pSRS_source = pSpatialRef_wgs84
                                pSRS_target = pSpatialRef_target
                    ...
                    if iFlag_resample_method == 2:  # zonal mean
                        ...
                        for pCell in aCell_in:
                            ...
                            if minX > dX_right or maxX < dX_left \
                                ...
                                # new method
                                #create a shapefile cutline for the cutline
                                sFilename_shapefile_cut = "/vsimem/tmp_polygon.shp"
                                pDataset_clip = pDriver_shapefile.CreateDataSource(sFilename_shapefile_cut)
                                pLayer_clip = pDataset_clip.CreateLayer("cutline", srs=pSpatialRef_wgs84, geom_type=ogr.wkbPolygon)
                                pLayer_clip.CreateFeature(ogr.Feature(pLayer_clip.GetLayerDefn()).SetGeometry(pPolygon))
                                pDataset_clip = None  #save and close the shapefile
                                pWrapOption = gdal.WarpOptions(cropToCutline=True,
                                                               cutlineDSName = sFilename_shapefile_cut , #use the shapefile as cutline if cutlineWKT does not work
                                                               xRes=abs(dPixelWidth),
                                                               yRes=abs(pPixelHeight),
                                                               dstSRS=pSpatialRef_target, format='MEM',
                                                               resampleAlg=sResampleAlg)
        ...
        for pCell in aCell_in:
            if pCell.lCellID in aCell_dict:
                ...
                aNeighbor_new = [
                    neighbor for neighbor in aNeighbor_land if neighbor in aCell_dict]
    ...
    def pyhexwatershed_generate_bash_script(self):
        ...
        if iFlag_unix == 1:
            sFilename_bash = os.path.join(
                str(Path(self.sWorkspace_output_hexwatershed)),  "run_hexwatershed.sh")
            ...
            sFilename_bash = os.path.join(
                str(Path(self.sWorkspace_output_hexwatershed)),  "run_hexwatershed.bat")
            ofs = open(sFilename_bash, 'w')
            sLine = 'cd ' + self.sWorkspace_output_hexwatershed + '\n'
            ofs.write(sLine)
            sLine = sFilename_executable + ' ' + sFilename_configuration + '\n'
    ...
    def pyhexwatershed_retrieve_domain_characteristics(self):
        """
        Read the number of outlets (nOutlet) from the domain characteristics file
        created by HexWatershed.

        Parameters:
        -----------
        filename : str
            Path to the domain characteristics file

        Returns:
        --------
        int
            Number of outlets (nOutlet)

        Raises:
        -------
        FileNotFoundError:
            If the domain characteristics file doesn't exist
        ValueError:
            If the nOutlet value couldn't be found or parsed properly
        """
        sFilename_domain_characteristics = os.path.join(
            self.sWorkspace_output_hexwatershed, 'domain.txt')
        try:
            with open(sFilename_domain_characteristics, 'r') as f:
                for sLine in f:
                    # Look for the line containing the outlet count
                    if "Total number of outlets:" in sLine:
                        # Extract the number after the colon
                        parts = sLine.split(":")
                        if len(parts) >= 2:
                            try:
                                nOutlet = int(parts[1].strip())
                                self.nOutlet = nOutlet
                            except ValueError:
                                raise ValueError("Failed to parse nOutlet value as integer")

        except FileNotFoundError:
            raise FileNotFoundError(f"Domain characteristics file not found: {sFilename_domain_characteristics}")
    ...
    def pyhexwatershed_export(self):
        ...
        if self.iFlag_global == 1:  # we do not have the polyline information
            ...
            if self.iFlag_multiple_outlet == 1:
                ...
                ptimer = pytimer()
                print('Started exporting flow direction')
                ptimer.start()
                self.pyhexwatershed_export_flow_direction()
                ...
                self.pyhexwatershed_export_flow_direction()
                ptimer.stop()
                print('Started exporting stream segment')
                ptimer.start()
                self.pyhexwatershed_export_stream_segment()
                ptimer.stop()
                # polygon
    ...
    def pyhexwatershed_export_stream_segment(self):
        ...
        if self.iFlag_global == 1:  # we do not output global scale stream segment yet
            ...
            if self.iFlag_multiple_outlet == 1:
                for iWatershed in range(1, nWatershed+1):
                    ...
                    export_json_to_geojson_polyline(
                        sFilename_stream_edge, sFilename_stream_edge_geojson)
                ...
                if iFlag_flowline == 1:  # we usually has one outlet
                    ...
                    sFilename_stream_edge = pBasin.sFilename_stream_edge_json
                    nCell = 0
                    with open(sFilename_stream_edge) as json_file:
                    ...
                    export_json_to_geojson_polyline_parallel(sFilename_stream_edge,
                                                             sFilename_stream_edge_geojson,
                                                             aVariable_json_in,
                                                             aVariable_geojson_out,
                                                             aVariable_type_out)
                    # add a performance improvement here, if there is too many cells,
                    # we will skip the following step
                    if nCell < 100000:
                        merge_stream_edge_to_stream_segment(sFilename_stream_edge_geojson,
                                                            sFilename_stream_segment_geojson,
                                                            pVertex_outlet)
                    ...
                    dummy_data = text_reader_string(
                        sFilename_watershed_characteristics, iSkipline_in=1, cDelimiter_in=':')
                    # find which line has the longitude and latitude
                    lIndex_longtitude = np.where(
                        dummy_data[:, 0] == 'Outlet longitude degree')[0]
                    lIndex_latitude = np.where(
                        dummy_data[:, 0] == 'Outlet latitude degree')[0]
                    dLongitude_outlet_longitude = float(
                        dummy_data[lIndex_longtitude, 1])
                    dLatitude_outlet_latitude = float(
                        dummy_data[lIndex_latitude, 1])
                    point = dict()
        ...
        sFilename_parquet = sFilename_stream_edge_geojson.replace(
            '.geojson', '.parquet')
        convert_vector_format(
            sFilename_stream_edge_geojson, sFilename_parquet)
        return
    ...
    def pyhexwatershed_export_flow_direction(self):
        ...
        if self.iFlag_global == 1:
            ...
            if self.iFlag_multiple_outlet == 1:
                ...
                aVariable_type_out = [2]
                export_json_to_geojson_polyline(
                    sFilename_json, sFilename_geojson, aVariable_json_in=aVariable_json,
                                            aVariable_geojson_out=aVariable_geojson,
                                            aVariable_type_out=aVariable_type_out)
                sFilename_parquet = sFilename_geojson.replace(
                    '.geojson', '.parquet')
    ...
    def pyhexwatershed_export_all_polygon_variables(self):
        ...
        if self.iMesh_type == 4:  # mpas mesh
            if self.iFlag_global == 1:
                # profile not enabled
                aVariable_json = ['dArea', 'dElevation',
                                  'dSlope_between', 'dDrainage_area']
                aVariable_geojson = [
                    'area', 'elevation', 'slope', 'drainage_area']

            else:
                aVariable_json = ['lSubbasin', 'lHillslope', 'dArea', 'dElevation', 'dSlope_between',
                                  'dDrainage_area', 'dDistance_to_watershed_outlet']  # profile not enabled
                aVariable_geojson = ['subbasin', 'hillslope', 'area',
                                     'elevation', 'slope', 'drainage_area', 'travel_distance']
        else:
            aVariable_json = ['lSubbasin', 'lHillslope', 'dArea', 'dElevation', 'dSlope_between',
                              'dDrainage_area', 'dDistance_to_watershed_outlet']  # profile not enabled
            aVariable_geojson = ['subbasin', 'hillslope', 'area',
        ...
        export_json_to_geojson_polygon_parallel(sFilename_json,
                                                sFilename_geojson,
                                                aVariable_json,
                                                aVariable_geojson,
                                                aVariable_type)
        ...
        if self.iFlag_multiple_outlet == 0:
            ...
            if self.iFlag_global == 1:
                ...
                # sFilename_geojson.replace('.geojson','_subbasin.parquet')
                sFilename_subbasin = os.path.join(sFolder, 'subbasin.parquet')
                merge_cell_to_polygon(
                    sFilename_parquet, sFilename_subbasin, 'subbasin')
                # sFilename_geojson.replace('.geojson','_hillslope.parquet')
                sFilename_hillslope = os.path.join(sFolder, 'hillslope.parquet')
                merge_cell_to_polygon(
                    sFilename_parquet, sFilename_hillslope, 'hillslope')
        return
