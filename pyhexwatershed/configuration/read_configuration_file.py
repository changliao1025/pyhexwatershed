...
def pyhexwatershed_read_configuration_file(sFilename_configuration_in,
    ...
    if iFlag_stream_burning_topology_in is not None:
        ...
        if "iFlag_stream_burning_topology" in aConfig:
            iFlag_stream_burning_topology = int(
    ...
    if iFlag_elevation_profile_in is not None:
        ...
        if "iFlag_elevation_profile" in aConfig:
    ...
    if iFlag_use_mesh_dem_in is not None:
        ...
        if "iFlag_use_mesh_dem" in aConfig:
    ...
    if dResolution_meter_in is not None:
        ...
        if "dResolution_meter" in aConfig:
        ...
        with open(oPyhexwatershed.sFilename_basins) as json_file:
            ...
            for i in range(oPyhexwatershed.nOutlet):
                ...
                dummy_basin['sWorkspace_output_basin'] = str(
                    Path(oPyhexwatershed.sWorkspace_output_hexwatershed) / sBasin)
                pBasin = pybasin(dummy_basin)
                oPyhexwatershed.aBasin.append(pBasin)
    else:
        #just add one here, or should we wait until the simulation is finished?
        pass
